	...
	switch ( inType ){
		...
		case 2:
			...
			if ( us_NewResult > us_OldResult ){
					zA=0;
					do{ motor[RIGHT] = 20; motor[LEFT] = -20; nxtDisplayCenteredBigTextLine(5, "Z: %d", zA);}while( zA >= -45 );
		...
		case 3:
			...
			stopSensors();
			if ( whatToDoNow > 50 ){
    		do{ motor[RIGHT] = 20; motor[LEFT] = -20; }while( zA >= -45 );
			}else{
    		do{ motor[RIGHT] = -20; motor[LEFT] = 20; }while( zA <= 45 );
			}
			runSensors();
